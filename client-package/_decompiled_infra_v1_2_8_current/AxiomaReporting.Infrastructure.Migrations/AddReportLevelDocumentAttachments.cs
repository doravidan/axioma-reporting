...
[Migration("20260510124639_AddReportLevelDocumentAttachments")]
public class AddReportLevelDocumentAttachments : Migration
{
	protected override void Up(MigrationBuilder migrationBuilder)
	{
		migrationBuilder.AddColumn<int>("ReportId", "DocumentAttachments", "int", null, null, rowVersion: false, null, nullable: true);
		migrationBuilder.CreateIndex("IX_DocumentAttachments_ReportId", "DocumentAttachments", "ReportId");
		migrationBuilder.AddForeignKey("FK_DocumentAttachments_Reports_ReportId", "DocumentAttachments", "ReportId", "Reports", null, null, "Id");
	}

	protected override void Down(MigrationBuilder migrationBuilder)
	{
		migrationBuilder.DropForeignKey("FK_DocumentAttachments_Reports_ReportId", "DocumentAttachments");
		migrationBuilder.DropIndex("IX_DocumentAttachments_ReportId", "DocumentAttachments");
		migrationBuilder.DropColumn("ReportId", "DocumentAttachments");
	}

	protected override void BuildTargetModel(ModelBuilder modelBuilder)
	{
		modelBuilder.HasAnnotation("ProductVersion", "6.0.36").HasAnnotation("Relational:MaxIdentifierLength", 128);
		...
		modelBuilder.Entity("AxiomaReporting.Core.Entities.TermsOfUseVersion", delegate(EntityTypeBuilder b)
		{
			...
			{
				Id = 1,
				BodyHtml = "<p>ברוכים הבאים למערכת דיווח הפעילות החודשית של סייט אנד סאונד.</p><p>השימוש במערכת מותנה בהסכמה לתנאי השימוש הבאים. אנא קראו אותם בעיון לפני האישור.</p><p>1. השימוש במערכת מיועד לעובדים מורשים בלבד, לצורך דיווח פעילות חודשית בלבד. אין להעביר את פרטי הכניסה לאדם אחר ואין להשתמש במערכת בשם משתמש שאינו שלך.</p><p>2. כל הנתונים המוזנים במערכת מהווים דיווח רשמי. עליך לוודא שכל המידע המוזן נכון, מדויק ומשקף את הפעילות שבוצעה בפועל. דיווח כוזב מהווה הפרה של נהלי הארגון.</p><p>3. הארגון רשאי לבצע ביקורת על הדיווחים בכל עת. דיווחים אשר אושרו ננעלים לעריכה ולא ניתן יהיה לשנותם ללא אישור מנהל.</p><p>4. המערכת שומרת יומן ביקורת של כל הפעולות. הגישה למידע מותנית בהרשאות ובהתאם לתפקיד המוגדר במערכת.</p><p>5. הסיסמה שלך אישית וסודית. יש להחליפה כל 90 יום ולא לחזור על 5 הסיסמאות האחרונות. לאחר 3 ניסיונות כניסה כושלים החשבון יינעל אוטומטית.</p><p>גרסה זו של תנאי השימוש מהווה גרסת ביניים — הגרסה המחייבת תפורסם על ידי מנהל המערכת דרך מסך 'תנאי שימוש' תחת תפריט הניהול.</p>",
		...
		modelBuilder.Entity("AxiomaReporting.Core.Entities.DocumentAttachment", delegate(EntityTypeBuilder b)
		{
			b.HasOne("AxiomaReporting.Core.Entities.Report", "Report").WithMany().HasForeignKey("ReportId")
				.OnDelete(DeleteBehavior.NoAction);
